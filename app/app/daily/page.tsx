...
const dailyChallenges = [
  {
    day: 0, title: "Generate a Keypair", difficulty: "Easy", category: "Accounts", xp: 50,
  ...
  {
    day: 1, title: "Lamports to SOL", difficulty: "Easy", category: "Utils", xp: 50,
  ...
  {
    day: 2, title: "Derive a PDA", difficulty: "Medium", category: "PDAs", xp: 100,
  ...
  {
    day: 3, title: "Send SOL Transfer", difficulty: "Medium", category: "Transactions", xp: 100,
  ...
  {
    day: 4, title: "Initialize Anchor Program", difficulty: "Hard", category: "Anchor", xp: 150,
  ...
  {
    day: 5, title: "Create Token Mint", difficulty: "Medium", category: "Tokens", xp: 100,
  ...
  {
    day: 6, title: "Verify Soulbound NFT", difficulty: "Hard", category: "NFTs", xp: 150,
...
function Countdown() {
  ...
  return (
    <div className="flex items-center gap-2">
      {[{ label: "HRS", value: timeLeft.hours }, { label: "MIN", value: timeLeft.minutes }, { label: "SEC", value: timeLeft.seconds }].map((t, i) => (
        <div key={i} className="text-center">
          <div className="font-display font-black text-xl md:text-3xl text-[#ff3366] w-12 md:w-16 h-12 md:h-16 border border-[#ff3366]/30 bg-[#ff3366]/5 flex items-center justify-center">
...
export default function DailyChallengePage() {
  ...
  const [activeTab, setActiveTab] = useState<"tests" | "editor">("editor");
    ...
    <div className="min-h-screen bg-[#020202]">
      ...
      <div className="border-b border-[#1a1a1a]">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12">
          ...
          <div className="flex items-start md:items-end justify-between flex-wrap gap-4 md:gap-6">
            <div>
            ...
            <div className="flex items-center gap-4 md:gap-8">
              <div className="text-right">
                <div className="font-display font-black text-2xl md:text-4xl text-[#ff3366]">🔥 {streak}</div>
              ...
              <div>
                <div className="text-[9px] font-mono text-[#444] uppercase tracking-widest mb-2">Next In</div>
                <Countdown />
              </div>
            </div>
          </div>
      ...
      <div className="border-b border-[#1a1a1a] bg-[#0a0a0a]">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-6">
          <div className="flex items-start md:items-center justify-between flex-wrap gap-4">
            <div className="flex items-start md:items-center gap-3 md:gap-4 flex-wrap">
              <span className={cn("px-3 py-1.5 border text-[10px] font-mono uppercase tracking-widest shrink-0", difficultyColor[challenge.difficulty])}>
                {challenge.difficulty}
              </span>
              <span className="text-[10px] font-mono text-[#444] uppercase">{challenge.category}</span>
              <div>
                <div className="font-display font-black text-xl md:text-2xl uppercase">{challenge.title}</div>
                <div className="text-xs font-mono text-[#555] mt-1">{challenge.description}</div>
              </div>
            </div>
            <div className="flex items-center gap-4 md:gap-6">
              <div className="text-right">
                <div className="font-display font-black text-2xl md:text-3xl text-[#9945ff]">+{challenge.xp}</div>
                <div className="text-[9px] font-mono text-[#444] uppercase">XP Reward</div>
              </div>
              {solved && (
                <div className="flex items-center gap-2 px-3 md:px-4 py-2 border border-[#14f195]/40 bg-[#14f195]/5 text-[#14f195] font-mono text-[10px] uppercase tracking-widest">
                  <CheckCircle className="w-3.5 h-3.5" />
                  <span className="hidden sm:block">Solved Today!</span>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="lg:hidden border-b border-[#1a1a1a] flex">
        <button onClick={() => setActiveTab("editor")}
          className={cn("flex-1 py-3 text-[10px] font-mono uppercase tracking-widest transition-colors",
            activeTab === "editor" ? "bg-[#0a0a0a] text-[#9945ff] border-b-2 border-b-[#9945ff]" : "text-[#444]"
          )}>
          Editor
        </button>
        <button onClick={() => setActiveTab("tests")}
          className={cn("flex-1 py-3 text-[10px] font-mono uppercase tracking-widest transition-colors",
            activeTab === "tests" ? "bg-[#0a0a0a] text-[#9945ff] border-b-2 border-b-[#9945ff]" : "text-[#444]"
          )}>
          Tests {tests.some(t => t.passing) ? "✓" : ""}
        </button>
      </div>

      {/* Editor */}
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6" style={{ height: "calc(100vh - 380px)", minHeight: "400px" }}>

          {/* Left: Tests + Output */}
          <div className={cn("border border-[#1a1a1a] flex flex-col overflow-hidden", activeTab === "tests" ? "flex" : "hidden lg:flex")}>
              ...
              <div className="space-y-2">
                {tests.map((t, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <div className={cn("w-5 h-5 flex items-center justify-center border text-[10px] font-mono shrink-0",
                      t.passing ? "border-[#14f195] text-[#14f195] bg-[#14f195]/10" : "border-[#333] text-[#333]"
                    )}>
                      {t.passing ? "✓" : "○"}
                    </div>
                    <span className={cn("text-xs font-mono", t.passing ? "text-[#14f195]" : "text-[#555]")}>{t.name}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex-1 p-4 md:p-6 overflow-y-auto">
              ...
              <div className="flex gap-2 flex-wrap">
                {Array.from({ length: 7 }).map((_, i) => {
                  ...
                  return (
                    <div key={i} className="text-center">
                      <div className={cn("w-9 h-9 flex items-center justify-center border font-mono text-[9px] mb-1",
                        isToday && solved ? "border-[#14f195] bg-[#14f195]/10 text-[#14f195]" :
                        completed ? "border-[#9945ff] bg-[#9945ff]/10 text-[#9945ff]" :
                        isToday ? "border-[#ff3366]/40 text-[#ff3366]" : "border-[#1a1a1a] text-[#333]"
                      )}>
              ...
              {output && (
                <div className="mt-4 border border-[#1a1a1a] bg-[#0a0a0a] p-4">
            ...
            <div className="border-t border-[#1a1a1a] px-4 md:px-6 py-3 md:py-4 flex items-center justify-between">
              <span className="text-[10px] font-mono text-[#444] uppercase hidden sm:block">{challenge.category} · TypeScript</span>
              <button onClick={runCode} disabled={running || solved}
                className={cn("flex items-center gap-2 px-4 md:px-6 py-2.5 font-mono text-[10px] uppercase tracking-widest transition-colors w-full sm:w-auto justify-center",
                  solved ? "bg-[#14f195]/10 border border-[#14f195]/40 text-[#14f195] cursor-default" : "bg-[#9945ff] text-white hover:bg-[#8835ef] disabled:opacity-50"
                )}>
                {solved ? <><CheckCircle className="w-3.5 h-3.5" /> Already Solved</> :
                 running ? "Running..." :
          ...
          {/* Right: Monaco Editor */}
          <div className={cn("border border-[#1a1a1a] flex flex-col overflow-hidden", activeTab === "editor" ? "flex" : "hidden lg:flex")}>
